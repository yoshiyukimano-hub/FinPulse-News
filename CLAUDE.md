...
│   ├── rate-history.html  # 金利履歴ヴューアー（?dataset=car でマイカーローン）
│   ├── data/rate-history.json # 住宅ローン金利履歴の正本
│   ├── data/car-loan-history.json # マイカーローン金利履歴の正本
│   ├── setup-connectors.md
...
- 金利履歴が比較する軸は「各機関の引下げ後・優遇下限の新規貸出金利」で統一する。店頭表示金利（基準金利）や引下げ幅は保存しない。サイトが「1.100〜1.150%」のように幅で示す項目は下限を採る。釧路信金REDと北海道労働金庫の変動3.325%、JA帯広かわにしの変動3.125%は、基準金利と同値だがこれが引下げ後の新規貸出金利であり、他行の1.5〜1.8%台と並んで高く見えても誤りではない（2026-08-27にサイトで確認済み）。
- マイカーローン金利は住宅ローンと同じ経路で扱う。上流の `car_loan_table`（変動・固定）を `docs/data/car-loan-history.json` へ積み上げ、表示は `rate-history.html?dataset=car` を新タブのiframeで読む。ビューアーは既知のデータセット名だけを受け付け、未知の値は住宅ローンに落とす（URLの値を取得先にしない）。
- 上流の `rate_contract` は version 1（住宅ローンのみ）と version 2（マイカーローン追加）の両方を受け入れる。2リポジトリの反映がずれても住宅ローンの取り込みを止めないための移行措置で、片側だけ更新された週も成果物は落ちない。
- マイカーローンで採用するのは各機関が数値で公表している引下げ後（優遇後）金利だけとする。金利をこちらで計算しない方針は住宅ローンと同じ。金利一覧ページだけを見て「基準金利しかない」と判断しないこと。帯広信金は商品ページに最優遇金利、北見信金はお知らせページの返済例に優遇後金利があり、一覧ページからは読めない（この見落としで2機関を一度対象外にした）。
- 適用条件が違うプランは1行にまとめず、商品行を分ける。北洋銀行は通常プラン（変動3.20・固定3.80）とゼロカーボン応援プラン（電気自動車・PHEVの新車をWEB契約する場合の変動2.80）で別行にしている。まとめると条件の違う金利が同じ行に並び、一般の車を買う人が最安値を自分に適用できると誤読するため。借入額で段階がある場合は代表値（北洋は1,000万円以下）を採り、残りは備考に書く。
- マイカーローンは表の上で「表示金利はすべて保証料込」と一度だけ断り、上乗せがある機関だけ機関名の下に注記する（`guarantee_note`）。2026-08-28時点で9機関すべてが保証料込を明記しており、北陸銀行のみオリコ保証時に年1.60%上乗せ。機関を追加するときは保証料の扱いを実ページで確認し、保証料別の機関が入る場合は凡例の書き方から見直す。住宅ローンは機関ごとに扱いが分かれる（JAは保証料別）ため凡例を出さない。
