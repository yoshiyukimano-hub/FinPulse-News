...
- 住宅ローンの「金利以外の条件」は `docs/data/loan-features.json` を正本とし、週次の収集経路には載せない。実ページと商品概要説明書を読んで人手で書く編集データで、12項目×15商品と量が多く、抽出の指示文と混ぜると週次実行の設定が壊れやすいため。マイカーローンの `guarantee_note` だけは上流 `config/banks.json` 由来のままにする（そちらは商品定義に付随する1行のため）。JSONの商品は金利履歴と一対一で、`product_id`・機関順・機関名・商品名の一致をテストで固定する。
- 住宅ローンは保証料の扱いが機関ごとに割れる（同じ北陸銀行でも定額型は別途・定率型は込み）ため、機関名の下に「保証料の扱い」と「事務手数料」の2行を出す。保証料が金利に含まれない商品だけ赤字にし、含む商品は灰色で目立たせない。条件データが読めなかった週は注記だけが消え、金利表は従来どおり出す。
