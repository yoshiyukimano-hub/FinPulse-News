...
│   ├── loan-features.html # 住宅ローンの条件比較ヴューアー
...
- 住宅ローンの「金利以外の条件」は `docs/data/loan-features.json` を正本とし、週次の収集経路には載せない。実ページと商品概要説明書を読んで人手で書く編集データで、11項目×15商品と量が多く、抽出の指示文と混ぜると週次実行の設定が壊れやすいため。マイカーローンの `guarantee_note` だけは上流 `config/banks.json` 由来のままにする（そちらは商品定義に付随する1行のため）。JSONの商品は金利履歴と一対一で、`product_id`・機関順・機関名・商品名の一致をテストで固定する。
- 住宅ローンは保証料の扱いが機関ごとに割れるため、機関名の下に「保証料の扱い」と「事務手数料」の2行を出す。扱いは `guarantee_status` の3値で表す。`included`（金利に含む）は灰色、`separate`（必ず別途）は赤、`conditional`（会員かどうかや審査で別途負担が生じうる／公式に記載がなく不明）は橙。2値の真偽では、金利内包を選べば上乗せ0％の北陸銀行の標準型や、会員だけ無料の北海道労働金庫を正しく表せず、赤の意味が薄まるため3値にした。条件データが読めなかった週は注記だけが消え、金利表は従来どおり出す。
- 条件比較の各セルには機関どうしの優劣（「最も安い」「唯一」など）を書かない。機関を1つ足すたびに全セルの再検証が必要になるうえ、2026-08-28の監査で「最も安い/高い/低い」の4件がデータ自身で反証できた。優劣の判断は表を見る人に委ねる。テストで文言を禁止している。
- 条件比較の一次情報は、各機関の自ドメインに加えて、その機関専用の公式ページを含む。JA帯広かわにしは自社サイトに条件をほとんど載せていないが、JAバンク公式のJAネットローンに同JA専用の商品概要（組合員資格・年齢・年収・限度額・期間・団信）があり、これを採用している。自ドメインだけに限ると、公開されている条件を利用者に伝えられない。
