...
> ## ⚠️ 廃止（使用禁止）
>
> このテンプレートは **2026-05-26（コミット `05750ec`）で GitHub Actions 週次実行へ移行済み**の旧構成の手順書です。
> 現行の週次自動化は `.github/workflows/weekly-news-report.yml` が収集からメール送信まで単独で完結しており、
> 本テンプレートが送る `repository_dispatch`（send-report）を受けるワークフローは既に存在しません。
> **このまま実行すると、成功表示のままメールが送信されない**うえ、以下の理由で安全上も使用できません。
>
> - repo スコープの PAT をプロンプトへ平文で貼り付ける指示（現行の Secrets / Deploy Key 運用と非互換）
> - 収集した外部ページ内容を Python ソースコードへ直接埋め込む指示（コード注入の余地）
> - スクレイピング取得内容へのプロンプトインジェクション防御指示なし
>
> 履歴参照用として残している。**再利用しないこと。**
