...
- 状態: **未実装（ユーザー承認待ち）**。実装時は本書の A評価 → 着手順に従う。
...
1. #24a: 日付切替時の機関絞り込みを「維持」に変えるか、「毎回リセット」を意図仕様として現状維持か。
2. #11: 今回の opus 塊で実施するか、requests バージョン更新タスクまで延期するか。
3. #2: テンプレは廃止注記で残す前提だが、削除（git 履歴のみ）を希望する場合は指示。
